...
本分支包含合成数据演示和 CSV 接入到主体审核的界面化切片。是否合入主分支，
以 GitHub PR 状态为准；CI 通过不等于商业上线批准。
...
- 接入自己的合成 CSV：`http://127.0.0.1:3180/ingest`；页面有可下载模板。
  [CSV 接入说明](docs/poc/csv-ingestion-v1.md)包含字段、限额、恢复入口和验收边界。
- 预置审核/生产/发布演示：[本地演示说明](docs/poc/local-demo.md)。
  新上传的数据不会自动绑定这个演示产品；`advance` 和 `verify` 也不是新 CSV 的通用处理命令。
- 停止并保留数据：`node deploy/demo/demo.mjs down`。服务仅开放本机回环控制台，
  不要用当前 POC 身份配置向公网或真实客户数据开放。
...
当前分支已具备：**可重复本地演示，以及 CSV → RAW 版本 → 实体解析 → 人工审核的界面入口**。
审核、加工和发布的真实服务验收与 CSV 上传验收为不同切片；测试报告必须对应确切提交。
上面的 Sprint 列表保留为建设路线，不再表示“尚未开始工程骨架”。
生产身份权限、多租户安全、通用加工向导和外部引擎的全链路验收仍需独立推进。
