...
- CurrentEntitlementGate 按 entitlement path 评估。DIRECT_USE 时，RightsDeclaration 的 current VERIFIED/validity/disposition、resource、consumer applicability、permitted purpose、allowed action、use scope 必须覆盖请求。DOWNSTREAM_AUTHORIZATION 时，RightsDeclaration 只需作为 current provenance/grant-authority source（VERIFIED/current/resource）；AuthorizationProvenanceBinding / grantor delegation chain 必须证明 **grantable purpose/action/scope** 覆盖 Authorization，Authorization 再覆盖 grantee/consumer、resource、purpose、action、scope、validity/status。**grantor 自己的 allowed/use permission 不需要匹配 grantee 的请求，也不能替代 grantable authority。** 任一 current binding/delegation/provenance 失效都 fail closed；
