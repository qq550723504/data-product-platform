...
当前 Core 的生产语义仍然是有边界输入产生 immutable DatasetVersion。上游即使采用 CDC / 消息流 / 流计算，进入质量、认证与交付链时也必须通过明确 snapshot / window / cut 发布新版本；当前不把无边界持续变化的数据直接解释为 DatasetVersion。

当前同样不提供数据资产会计确认、计量或入表能力。Cost / Rights / Evidence 是 Core 业务事实，不自动生成会计科目、凭证、估值、摊销、减值、资产负债表结论，也不承担现实世界法律权利裁定。
