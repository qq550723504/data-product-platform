-- Refuse to erase immutable Gold certification proof once issued.
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM dataset_certification
        WHERE gold_production_binding_id IS NOT NULL
           OR annotation_snapshot_id IS NOT NULL
    ) THEN
        RAISE EXCEPTION 'refusing rollback: DatasetCertification contains Gold production proof';
    END IF;
END
$$;

-- Restore the typed-rights-aware certification trigger before removing Gold
-- proof columns. Typed required resources remain part of #207 rights history.
...
DROP INDEX IF EXISTS idx_dataset_certification_gold_binding;

ALTER TABLE dataset_certification
    DROP CONSTRAINT IF EXISTS ck_dataset_certification_gold_proof,
    DROP COLUMN gold_production_binding_root_hash,
    DROP COLUMN annotation_taxonomy_content_sha256,
    DROP COLUMN annotation_schema_content_sha256,
    DROP COLUMN annotation_snapshot_root_hash,
    DROP COLUMN annotation_snapshot_id,
    DROP COLUMN gold_production_binding_id;
