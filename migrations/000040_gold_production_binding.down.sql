...
LOCK TABLE gold_production_binding, gold_production_member IN ACCESS EXCLUSIVE MODE;

DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM gold_production_binding LIMIT 1)
...
DROP FUNCTION IF EXISTS guard_gold_binding_delete();
...
DROP FUNCTION IF EXISTS guard_gold_member_mutation();
