...
CREATE TABLE annotation_campaign (
    ...
    input_checksum_sha256           varchar(64),
...
RETURNS trigger AS $campaign_cmd$
DECLARE
...
CREATE TABLE annotation_task (
    ...
    primary_annotator_ref       varchar(255) NOT NULL,
...
BEGIN
    ...
    IF OLD.status='DRAFT' AND NEW.status='ACTIVE' THEN
        ...
        IF NEW.expected_task_count IS NULL OR NEW.expected_task_count <> task_count OR task_count < 1
           OR NEW.task_manifest_hash IS NULL
           OR NEW.input_checksum_sha256 IS NULL
           OR NEW.input_checksum_sha256 !~ '^[0-9a-f]{64}
    ...
    IF OLD.status='DRAFT' AND NEW.status='CANCELLED' THEN
        IF NEW.input_checksum_sha256 IS DISTINCT FROM OLD.input_checksum_sha256
           OR NEW.cancelled_at IS NULL OR NEW.revision <> OLD.revision + 1 THEN
    ...
    IF OLD.status='ACTIVE' AND NEW.status='SEALED' THEN
        IF NEW.input_checksum_sha256 IS DISTINCT FROM OLD.input_checksum_sha256
           OR NEW.sealed_at IS NULL OR NEW.revision <> OLD.revision + 1 THEN
...
DECLARE
    ...
    task_primary_annotator_ref varchar(255);
    corrected_task uuid;
BEGIN
    ...
    SELECT workspace_id, campaign_id, status, primary_annotator_ref
      INTO task_workspace, task_campaign, task_status, task_primary_annotator_ref
    ...
    IF NEW.corrected_from_result_id IS NULL
       AND NEW.author_ref IS DISTINCT FROM task_primary_annotator_ref THEN
        RAISE EXCEPTION 'annotation result author does not match primary annotator';
    END IF;
...
DECLARE
    ...
    selected_author_ref varchar(255);
BEGIN
    SELECT status INTO campaign_status
      FROM annotation_campaign
     WHERE id=NEW.campaign_id
    ...
    IF NEW.selected_result_id IS NOT NULL THEN
        SELECT task_id, corrected_from_result_id, author_ref
          INTO selected_task, selected_corrected_from, selected_author_ref
    ...
    IF NEW.outcome='CORRECT' AND (
        selected_corrected_from IS DISTINCT FROM NEW.reviewed_result_id
        OR selected_author_ref IS DISTINCT FROM NEW.reviewer_ref
    ) THEN
        RAISE EXCEPTION 'corrected annotation result must be authored by reviewer and point to reviewed result';
    END IF;
...
$$ LANGUAGE plpgsql;
    ...
    CONSTRAINT ck_annotation_task_primary_annotator CHECK (length(btrim(primary_annotator_ref)) > 0)
);
...
$$ LANGUAGE plpgsql;
    ...
    CONSTRAINT ck_annotation_campaign_input_checksum CHECK (
        input_checksum_sha256 IS NULL OR input_checksum_sha256 ~ '^[0-9a-f]{64}
);
