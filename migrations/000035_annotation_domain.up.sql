...
BEGIN
    IF NEW.id IS DISTINCT FROM OLD.id
       ...
       OR (
            ...
            AND NEW.input_checksum_sha256 IS DISTINCT FROM OLD.input_checksum_sha256
       )
...
BEGIN
    ...
    IF NEW.outcome='CORRECT'
       AND (
           selected_corrected_from IS DISTINCT FROM NEW.reviewed_result_id
           OR selected_author_ref IS DISTINCT FROM NEW.reviewer_ref
       ) THEN
        RAISE EXCEPTION 'corrected annotation result must point to reviewed result and be authored by reviewer';
    END IF;
