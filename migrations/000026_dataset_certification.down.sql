...
DROP INDEX idx_cost_allocation_certification_disposition;
DROP INDEX idx_cost_allocation_dataset_certification;
ALTER TABLE cost_allocation DROP CONSTRAINT ck_cost_allocation_single_subject;
ALTER TABLE cost_allocation DROP COLUMN certification_disposition_id;
ALTER TABLE cost_allocation DROP COLUMN dataset_certification_id;
...
-- Restore the #137 guard before removing this migration's subject columns.
...
DROP TRIGGER trg_certification_disposition_immutable ON certification_disposition;
DROP TRIGGER trg_certification_disposition_references ON certification_disposition;
DROP FUNCTION prevent_certification_disposition_mutation();
DROP FUNCTION validate_certification_disposition_references();
DROP TRIGGER trg_dataset_certification_immutable ON dataset_certification;
DROP TRIGGER trg_dataset_certification_references ON dataset_certification;
DROP FUNCTION prevent_dataset_certification_mutation();
DROP FUNCTION validate_dataset_certification_references();
DROP TABLE certification_disposition;
DROP TABLE dataset_certification;
