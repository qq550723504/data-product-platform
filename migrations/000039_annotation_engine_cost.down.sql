LOCK TABLE cost_allocation, annotation_engine_attempt IN ACCESS EXCLUSIVE MODE;

DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM cost_allocation
         WHERE annotation_engine_attempt_id IS NOT NULL
         LIMIT 1
    ) THEN
        RAISE EXCEPTION 'cannot rollback annotation engine cost allocation with history';
    END IF;
END;
$$;

DROP INDEX IF EXISTS idx_cost_allocation_annotation_engine_attempt;
ALTER TABLE cost_allocation DROP CONSTRAINT ck_cost_allocation_single_subject;
ALTER TABLE cost_allocation DROP COLUMN annotation_engine_attempt_id;
...
BEGIN
    ...
    ELSE
        SELECT workspace_id INTO subject_workspace FROM annotation_review_attempt WHERE id=NEW.annotation_review_attempt_id;
    END IF;
