...
-- Gold remains a normal immutable DatasetVersion. These tables freeze the exact
-- build request and annotation/producer facts used to produce that version.
...
CREATE INDEX idx_gold_build_request_workspace
    ON gold_build_request(workspace_id, created_at DESC, execution_id);
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM execution_input ei
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM gold_build_request r
...
CREATE OR REPLACE FUNCTION guard_gold_member_insert()
RETURNS trigger AS $$
DECLARE
    binding_snapshot uuid;
    binding_status varchar(16);
    frozen_task record;
    frozen_decision record;
    frozen_output uuid;
    result_row record;
BEGIN
    SELECT annotation_snapshot_id, status
      INTO binding_snapshot, binding_status
      FROM gold_production_binding
     WHERE id=NEW.binding_id
     FOR UPDATE;
    IF binding_status IS DISTINCT FROM 'BUILDING' THEN
        RAISE EXCEPTION 'gold production binding membership is finalized';
    END IF;

    SELECT source_item_ref, source_content_sha256
      INTO frozen_task
      FROM annotation_snapshot_task
     WHERE snapshot_id=binding_snapshot AND task_id=NEW.task_id;
    IF frozen_task.source_item_ref IS DISTINCT FROM NEW.source_item_ref
       OR frozen_task.source_content_sha256 IS DISTINCT FROM NEW.source_content_sha256 THEN
        RAISE EXCEPTION 'gold production member does not match frozen snapshot task';
    END IF;

    SELECT decision_id, outcome, reviewed_result_id, selected_result_id
      INTO frozen_decision
      FROM annotation_snapshot_decision
     WHERE snapshot_id=binding_snapshot AND task_id=NEW.task_id;
    IF frozen_decision.decision_id IS DISTINCT FROM NEW.decision_id
       OR frozen_decision.outcome IS DISTINCT FROM NEW.outcome
       OR frozen_decision.reviewed_result_id IS DISTINCT FROM NEW.reviewed_result_id
       OR frozen_decision.selected_result_id IS DISTINCT FROM NEW.selected_result_id THEN
        RAISE EXCEPTION 'gold production member does not match frozen snapshot decision';
    END IF;

    SELECT selected_result_id INTO frozen_output
      FROM annotation_snapshot_output
     WHERE snapshot_id=binding_snapshot AND task_id=NEW.task_id;

    IF NEW.outcome IN ('ACCEPT','CORRECT') THEN
        IF frozen_output IS DISTINCT FROM NEW.selected_result_id THEN
            RAISE EXCEPTION 'gold production selected result does not match frozen snapshot output';
        END IF;
        SELECT task_id, canonical_payload_sha256
          INTO result_row
          FROM annotation_result
         WHERE id=NEW.selected_result_id;
        IF result_row.task_id IS DISTINCT FROM NEW.task_id
           OR result_row.canonical_payload_sha256 IS DISTINCT FROM NEW.selected_result_sha256 THEN
            RAISE EXCEPTION 'gold production selected result identity/hash mismatch';
        END IF;
    ELSIF frozen_output IS NOT NULL THEN
...
FOR EACH ROW EXECUTE FUNCTION guard_gold_member_insert();

CREATE OR REPLACE FUNCTION prevent_gold_member_mutation()
RETURNS trigger AS $$
BEGIN
    RAISE EXCEPTION 'gold production member is immutable history';
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_gold_member_update
BEFORE UPDATE ON gold_production_member
FOR EACH ROW EXECUTE FUNCTION prevent_gold_member_mutation();
...
BEGIN
    IF OLD.status <> 'BUILDING' OR NEW.status <> 'FINALIZED' THEN
        RAISE EXCEPTION 'gold production binding only supports BUILDING -> FINALIZED';
    END IF;
    ...
    SELECT e.workspace_id, e.workflow_version_id, e.output_dataset_id, e.status
      INTO execution_row
      FROM execution e
     WHERE e.id=NEW.execution_id;
    IF execution_row.workspace_id IS DISTINCT FROM NEW.workspace_id
       OR execution_row.workflow_version_id IS DISTINCT FROM NEW.workflow_version_id
       OR execution_row.status NOT IN ('RUNNING','SUCCEEDED') THEN
        RAISE EXCEPTION 'gold production execution identity mismatch';
    END IF;
    ...
    SELECT c.schema_content_sha256, c.taxonomy_content_sha256, c.rubric_content_sha256,
           c.renderer_content_sha256, c.review_policy_content_sha256
      INTO campaign_row
      FROM annotation_campaign c
     WHERE c.id=NEW.annotation_campaign_id;
    IF campaign_row.schema_content_sha256 IS DISTINCT FROM NEW.schema_content_sha256
...
CREATE OR REPLACE FUNCTION prevent_gold_binding_delete()
RETURNS trigger AS $$
BEGIN
...
FOR EACH ROW EXECUTE FUNCTION prevent_gold_binding_delete();
