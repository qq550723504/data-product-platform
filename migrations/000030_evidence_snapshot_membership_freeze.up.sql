...
-- Existing rows predate the lifecycle but are already historical snapshots.
UPDATE evidence_snapshot
   SET status='FINALIZED'
 WHERE status IS NULL;

-- Steady-state inserts must enter through BUILDING. The deferred commit guard
-- below prevents a caller that omits explicit finalization from committing.
ALTER TABLE evidence_snapshot
