...
func TestEnterpriseActivityNativeWorkerProducesCuratedDataset(t *testing.T) {
	...
	for _, row := range rows {
		if _, exists := row["company_name"]; exists {
			t.Fatalf("V1 CURATED output leaked non-contract company_name column: %#v", row)
		}
	}
	...
	if frozenRow == nil {
		t.Fatalf("native CURATED output missing frozen resolution entity %s", originalDecision.EntityID)
	}
	if frozenRow["activity_score"] != "96.01" {
		t.Fatalf("frozen resolution entity activity_score = %q, want 96.01", frozenRow["activity_score"])
	}
...
func assertOutputSet(t *testing.T, rows []map[string]string, want map[string]int) {
	...
	if len(got) != len(want) {
		t.Fatalf("output result classes = %#v, want %#v", got, want)
	}
	for key, count := range want {
		if got[key] != count {
			t.Fatalf("output class %q count = %d, want %d; all=%#v", key, got[key], count, got)
		}
	}
}
