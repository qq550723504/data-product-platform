		...
		SELECT NOT EXISTS(
			...
			WHERE NOT EXISTS (
				...
				WHERE p.declaration_id=$1 AND p.permission_kind='GRANT' AND p.action=requested_action
				  ...
				  AND EXISTS (SELECT 1 FROM rights_declaration_scope sc WHERE sc.permission_id=p.id AND ((sc.scope_type='ALL_RESOURCE' AND sc.scope_ref=$6) OR (sc.scope_type=$4 AND sc.scope_ref=$5)))
			)
		)`, binding.DeclarationID, actions, purpose, scopeType, scopeRef, binding.DataResourceID.String()).Scan(&supported); err != nil || !supported {
		return domain.ErrInvalidBinding
	}
		...
		if err := tx.QueryRow(ctx, `
			SELECT d.data_resource_id,
			       EXISTS(
					...
					WHERE v.declaration_id=d.id AND v.outcome='VERIFIED' AND v.occurred_at <= $2
...
func checkCurrentEntitlement(ctx context.Context, q queryer, request domain.EntitlementRequest) (domain.EntitlementDecision, error) {
	...
	query := `
		...
		JOIN rights_declaration_verification v ON v.declaration_id=d.id AND v.outcome='VERIFIED' AND v.occurred_at <= $6
