...
func TestBuildGoldCSVPreservesSourceOrderAndSkipsRejectedTask(t *testing.T) {
	...
	frozen := []goldinfra.FrozenMember{
		{
			TaskID: uuid.New(), SourceItemRef: "row:2", SourceContentSHA256: strings.Repeat("b", 64),
			DecisionID: uuid.New(), Outcome: "REJECT",
		},
		{
			TaskID: uuid.New(), SourceItemRef: "row:1", SourceContentSHA256: strings.Repeat("a", 64),
