...
import { evaluateReleaseReadiness, requiredReadinessGates } from "@/lib/release-readiness";
...
function PublishForm({ productId, item, enabled }: { productId: string; item: ReleasePanelItem; enabled: boolean }) {
  ...
  const evaluation = evaluateReleaseReadiness(item.release.status, item.readiness);
  const publishable = enabled && evaluation.ready;
  ...
  return (
    <form action={action} style={{ marginTop: 16 }}>
      ...
      {enabled && !publishable && item.release.status !== "PUBLISHED" ? (
        <small style={{ display: "block", marginTop: 8 }}>Core Readiness 未完整通过：{evaluation.problems.join(" · ") || "UNKNOWN"}。</small>
      ) : null}
...
export function ProductReleasePanel({ productId, items, actionsEnabled }: { productId: string; items: ReleasePanelItem[]; actionsEnabled: boolean }) {
  ...
  return (
    <div style={{ display: "grid", gap: 18 }}>
      {items.map((item) => {
        const evaluation = evaluateReleaseReadiness(item.release.status, item.readiness);
        const runtimeBlockers = Array.isArray(item.readiness.blockers) ? item.readiness.blockers.filter((value) => typeof value === "string") : [];
        return (
          <article className="detail-card" key={item.release.id}>
            ...
            <div className="metric-grid" style={{ marginTop: 16 }}>
              {requiredReadinessGates.map((gate) => (
                <div className="metric-card" key={gate}>
                  <span>{gateLabels[gate] ?? gate}</span>
                  <Badge value={item.readiness.checks?.[gate] ?? "MISSING"} />
                </div>
              ))}
            </div>

            {evaluation.ready ? (
              <div className="callout" style={{ marginTop: 16 }}><strong>所有 Release Readiness 条件已完整通过</strong></div>
            ) : (
              <div className="callout callout-warn" style={{ marginTop: 16 }}>
                <strong>当前 Release 不可发布</strong>
                {runtimeBlockers.length > 0 ? <p>Core Blockers：{runtimeBlockers.join(" · ")}</p> : null}
                <p>校验：{evaluation.problems.join(" · ") || "READINESS_INVALID"}</p>
              </div>
            )}
