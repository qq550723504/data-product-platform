...
test(`real Core browser phase: ${phase}`, async ({ page }, testInfo) => {
  ...
  if (phase === "review") {
    ...
    await expect(page).toHaveURL(new RegExp(`${certifiedPath}(?:\\?.*)?import { test, expect } from "@playwright/test";
import { readFile } from "node:fs/promises";
...
test(`real Core browser phase: ${phase}`, async ({ page }, testInfo) => {
  ...
  if (phase === "review") {
    ...
    // Version facts shown in the UI must be the same immutable CURATED object
    // whose bytes/checksum and release trace were independently verified by Go.
